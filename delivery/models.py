...
def get_business_date(dt=None):
    """
    Returns the business date for a given datetime.
    Business day runs from 03:00 AM to 02:59:59 AM next day.
    """
    if dt is None:
...
class WalkInTokenCounter(models.Model):
    """
    Tracks the last walk-in token number per shop per business day.
    Unique constraint ensures one counter per (shop, business_date).
    """
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['shop', 'business_date']),
        ]
    ...
    def get_next_token(cls, shop):
        """
        Thread-safe token generation using select_for_update().
        Returns the next 4-digit token string (e.g. '00018').
        """
...
class DeliveryBoyProfile(models.Model):
    ...
    class Meta:
        indexes = [
            models.Index(fields=['shop', 'is_online', 'is_available']),
        ]
...
class Parcel(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.parcel_number:
            # Generate parcel number: PAR-{shop_code}-{timestamp}
...
class DeliveryAssignment(models.Model):
    ...
    # Reference to previous assignment (for reassignment history)
